...
| Use actions | **off** | Where a case's task button lands. The button reads the same either way — "3 open tasks" — and only the destination changes: off, it opens the decision directly, which is the shorter path from a case already in front of you; on, it goes to the Actions queue, where someone working a shift would start. Off by default, because from a case you are already looking at, a queue is a detour. |
