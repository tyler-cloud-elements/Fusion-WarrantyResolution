...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ==========================================================
     ...
     ========================================================== */
  .viewsw{display:flex;align-items:center;gap:8px;margin:14px 0 0}
  .viewsw .sw{display:flex;gap:2px;background:#eceff3;border:1px solid var(--line);border-radius:8px;padding:2px}
  .viewsw button{font-family:var(--pmono);font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;font-weight:600;
    padding:5px 12px;border-radius:6px;border:0;background:transparent;color:var(--muted);cursor:pointer}
  .viewsw button[aria-pressed="true"]{background:#fff;color:var(--teal);box-shadow:0 1px 2px rgba(20,28,36,.10)}
  .viewsw .swnote{font-size:10.5px;color:var(--muted)}
  ...
  .mw-bar{width:120px;height:6px;border-radius:100px;background:#e3ecec;overflow:hidden}
  .mw-bar i{display:block;height:100%;background:#0db4b9}
  ...
  .mw-c.case{border-top:2px solid #0db4b9}
  .mw-c.wf{border-top:2px solid #3b6fe0}
  ...
  .est .d.hi{border-color:#0db4b9;box-shadow:0 0 0 2px rgba(13,180,185,.16)}
  ...
  /* ==========================================================
     AUTOMATION HUB - process definitions, the governed record.
     Real Apollo tokens from Max's prototype (project-blueprint),
     so this reads as a third product, not as the Case App and
     not as Cartographer's own canvas.
     ========================================================== */
  :root{
    --ah-ink:#182027; --ah-ink-500:#6b7882; --ah-ink-300:#cfd8dd; --ah-ink-150:#f4f5f7;
    --ah-line:#e1e2e4; --ah-canvas:#fafafa; --ah-orange:#fa4616; --ah-blue:#0067df;
    --ah-blue-100:#dae8fa; --ah-green:#038108; --ah-green-100:#eeffe5;
    --ah-yellow:#ffb40e; --ah-yellow-100:#fff3db; --ah-purple:#933692; --ah-purple-100:#eee1ee;
  }
  .ah{flex:1;min-height:0;display:flex;background:#fff;color:var(--ah-ink);font-family:var(--sans);
      font-size:13.5px;overflow:hidden}
  .ah-side{width:176px;flex:none;border-right:1px solid var(--ah-line);padding:12px 9px;
  ...
  .ah-side .brand i{width:18px;height:18px;border-radius:5px;background:var(--ah-orange);flex:none;
                    display:block}
  .ah-nav{font-size:12px;color:#414244;padding:5px 8px;border-radius:6px;white-space:nowrap;
          overflow:hidden;text-overflow:ellipsis}
  .ah-nav.on{background:var(--ah-ink-150);color:var(--ah-ink);font-weight:600}
  .ah-main{flex:1;min-width:0;display:flex;flex-direction:column;overflow:hidden}
  .ah-top{flex:none;padding:11px 20px 0}
  .ah-crumb{display:flex;align-items:center;gap:7px;font-size:12px;color:var(--ah-ink-500)}
  .ah-crumb b{color:var(--ah-ink);font-weight:600}
  .ah-crumb .sep{color:var(--ah-ink-300)}
  ...
  .ah-btn{display:inline-flex;align-items:center;gap:5px;height:27px;padding:0 11px;border-radius:6px;
          border:1px solid var(--ah-line);font-size:12px;color:var(--ah-ink);background:#fff;white-space:nowrap}
  .ah-sub{font-size:12px;color:var(--ah-ink-500)}
  ...
  .ah-pill.phase{background:var(--ah-yellow-100);color:#9e6100}
  .ah-pill.ok{background:var(--ah-green-100);color:var(--ah-green)}
  ...
  .ah-gov{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:11px 22px;
          padding:14px 20px;margin:13px 0 0;border-top:1px solid var(--ah-line);
          border-bottom:1px solid var(--ah-line)}
  .ah-gov div span{display:block;font-family:var(--pmono);font-size:8px;letter-spacing:.11em;
                   text-transform:uppercase;color:var(--ah-ink-500);margin-bottom:2px}
  .ah-gov div b{font-size:12.5px;font-weight:500}
  .ah-gov div b.mono{font-family:var(--pmono);font-size:11.5px;letter-spacing:.02em;color:var(--ah-ink)}
  .ah-tabs{flex:none;display:flex;gap:2px;padding:11px 20px 0}
  .ah-tabs span{font-size:12px;font-weight:500;color:var(--ah-ink-500);padding:6px 13px;border-radius:8px 8px 0 0}
  .ah-tabs span.on{background:var(--ah-ink-150);color:var(--ah-ink);font-weight:600}
  .ah-doc{flex:1;min-height:0;display:grid;grid-template-columns:216px minmax(0,1fr);gap:0;
          border-top:1px solid var(--ah-line);overflow:hidden}
  .ah-toc{border-right:1px solid var(--ah-line);padding:13px 12px;overflow:hidden;background:#fff}
  ...
  .ah-toc div.sub{padding-left:20px;font-size:11px;color:var(--ah-ink-500)}
  ...
  .ah-body .dsub{font-size:11.5px;color:var(--ah-ink-500);margin-bottom:12px}
  ...
  .ah-body p b{font-weight:600;color:var(--ah-ink)}
  .ah-kpi{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:11px 0}
  .ah-kpi .k{border:1px solid var(--ah-line);border-radius:8px;padding:9px 12px}
  .ah-kpi .k span{display:block;font-family:var(--pmono);font-size:8px;letter-spacing:.11em;
                  text-transform:uppercase;color:var(--ah-ink-500);margin-bottom:3px}
  .ah-kpi .k b{font-size:17px;font-weight:600;letter-spacing:-.3px}
  .ah-kpi .k u{display:block;text-decoration:none;font-size:10.5px;color:var(--ah-ink-500);margin-top:2px}
  .ah-sign{border:1px solid var(--ah-line);border-radius:8px;padding:11px 13px;margin-top:11px}
  ...
  .ah-sr .av{width:20px;height:20px;flex:none;border-radius:50%;background:var(--ah-ink-150);
             color:#414244;font-family:var(--pmono);font-size:8.5px;font-weight:600;display:flex;
             align-items:center;justify-content:center}
  .ah-sr .rl{flex:1;min-width:0;color:var(--ah-ink-500);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .ah-sr .rl b{color:var(--ah-ink);font-weight:600}


  /* ==========================================================
     WARRANTY RESOLUTION CONSOLE - the human decision, as a console.
     Alin on the 25 Aug call: "The Coded app is basically mostly a
     form... Similar to a vertical solution, you basically have a
     console to operate inside, not just a small app with a form."
     So the three evidence sources Tuan named are brought forward
     and readable rather than linked. Evidence scrolls, the decision
     does not, and the signal-capture rail never decides anything.
     ========================================================== */
  .cn{position:absolute;inset:0;display:flex;flex-direction:column;background:var(--p-bg);
      font-family:var(--sans);color:var(--p-ink);overflow:hidden}
  .cn-top{flex:none;display:flex;align-items:center;gap:11px;padding:9px 17px;background:#fff;
          border-bottom:1px solid var(--p-line)}
  .cn-top .lg{width:24px;height:24px;flex:none;border-radius:6px;background:var(--p-teal);color:#fff;
              font-family:var(--serif);font-size:12px;display:flex;align-items:center;justify-content:center}
  .cn-top .nm{font-family:var(--serif);font-size:14px;font-weight:500;white-space:nowrap}
  .cn-top .who{margin-left:auto;display:flex;align-items:center;gap:8px;font-size:11.5px}
  .cn-top .who u{display:block;text-decoration:none;font-size:9.5px;color:var(--p-mute)}
  .cn-top .who i{width:24px;height:24px;border-radius:50%;background:var(--p-teal-50);color:var(--p-teal-700);
                 font-family:var(--pmono);font-size:9px;font-weight:600;font-style:normal;display:flex;
                 align-items:center;justify-content:center}
  .cn-head{flex:none;padding:11px 17px 12px;background:#fff;border-bottom:1px solid var(--p-line)}
  .cn-head .r1{display:flex;align-items:baseline;gap:9px;flex-wrap:wrap}
  .cn-head h4{font-family:var(--serif);font-size:17px;font-weight:500;letter-spacing:-.015em}
  .cn-facts{display:flex;gap:22px;flex-wrap:wrap;margin-top:8px}
  .cn-facts div span{display:block;font-family:var(--pmono);font-size:8px;letter-spacing:.11em;
                     text-transform:uppercase;color:var(--p-mute);margin-bottom:1px}
  .cn-facts div b{font-size:11.5px;font-weight:500}
  .cn-body{flex:1;min-height:0;display:grid;grid-template-columns:minmax(0,1fr) 300px}
  .cn-col{min-width:0;display:grid;grid-template-rows:minmax(0,1fr) auto;overflow:hidden}
  .cn-scroll{min-height:0;overflow:hidden;padding:13px 17px 0;display:flex;flex-direction:column;gap:10px}
  .cn-why{background:var(--p-teal-surf);border:1px solid var(--p-teal-line);border-radius:9px;padding:10px 13px}
  .cn-why b{display:block;font-size:12px;font-weight:600;margin-bottom:2px}
  .cn-why p{font-size:11.5px;line-height:1.5;color:#2c3a52}
  .cn-ev{border:1px solid var(--p-line);border-radius:9px;background:#fff;overflow:hidden}
  .cn-ev .h{display:flex;align-items:center;gap:9px;padding:8px 12px;background:var(--p-paper);
            border-bottom:1px solid var(--p-line)}
  .cn-ev .h .tri{font-family:var(--pmono);font-size:9px;color:var(--p-mute)}
  .cn-ev .h .ext{font-family:var(--pmono);font-size:7.5px;font-weight:600;letter-spacing:.08em;
                 text-transform:uppercase;background:var(--p-muted-bg);color:var(--p-mute);
                 border-radius:3px;padding:2px 5px}
  .cn-ev .h .t{flex:1;min-width:0;font-size:11.5px;font-weight:600}
  .cn-ev .in{padding:10px 12px}
  .cn-ev p{font-size:11px;line-height:1.55;color:#2c3a52}
  .cn-ev p b{font-weight:600;color:var(--p-ink)}
  table.cn-t{width:100%;border-collapse:collapse;font-size:10.5px}
  table.cn-t th{font-family:var(--pmono);font-size:7.5px;letter-spacing:.1em;text-transform:uppercase;
                color:var(--p-mute);font-weight:600;text-align:left;padding:5px 8px;
                border-bottom:1px solid var(--p-line);background:var(--p-paper)}
  table.cn-t td{padding:5px 8px;border-bottom:1px solid #eef1f5}
  table.cn-t tr:last-child td{border-bottom:0}
  table.cn-t td.k{font-family:var(--pmono);font-size:10px;color:var(--p-mute)}
  table.cn-t tr.flag td{background:#fdf6f6}
  table.cn-t tr.flag td:first-child{box-shadow:inset 3px 0 0 var(--p-red-ink)}
  .cn-delta{font-family:var(--pmono);font-size:10px;font-weight:600;color:var(--p-red-ink)}
  .cn-log{font-family:var(--pmono);font-size:10px;line-height:1.8;color:#2c3a52;white-space:pre-wrap}
  .cn-log .hi{background:#fdebec;color:var(--p-red-ink);font-weight:600;border-radius:3px;padding:0 3px}
  /* the decision: pinned, and the only one on the screen */
  .cn-deck{flex:none;padding:0 17px 13px;background:var(--p-bg);
           box-shadow:0 -13px 20px -13px rgba(16,33,61,.18)}
  .cn-dec{border:1px solid var(--p-teal-line);border-radius:9px;background:#fff;overflow:hidden}
  .cn-dec .dh{display:flex;align-items:center;gap:8px;padding:9px 13px;background:var(--p-teal-surf);
              border-bottom:1px solid var(--p-teal-line)}
  .cn-dec .dh b{font-size:12px;font-weight:600}
  .cn-dec .db{padding:10px 13px 11px}
  .cn-opt{display:flex;gap:9px;align-items:flex-start;border:1px solid var(--p-line-2);border-radius:8px;
          padding:7px 11px;margin-bottom:5px;background:#fff}
  .cn-opt:last-of-type{margin-bottom:0}
  .cn-opt .rad{width:13px;height:13px;flex:none;margin-top:2px;border-radius:50%;border:1.5px solid var(--p-line-2)}
  .cn-opt.on{border-color:var(--p-teal);background:var(--p-teal-surf);box-shadow:0 0 0 2px rgba(11,87,208,.10)}
  .cn-opt.on .rad{border-color:var(--p-teal);background:var(--p-teal);box-shadow:inset 0 0 0 2.5px #fff}
  .cn-opt b{display:block;font-size:11.5px;font-weight:600}
  .cn-opt em{display:block;font-style:normal;font-size:10.5px;color:var(--p-mute);margin-top:1px}
  .cn-opt.on b::after{content:" · Selected";font-family:var(--pmono);font-size:8px;letter-spacing:.08em;
                      text-transform:uppercase;color:var(--p-teal-700);font-weight:600}
  .cn-rat{margin-top:8px}
  .cn-rat .box{border:1px solid var(--p-line-2);border-radius:7px;padding:7px 10px;font-size:11px;
               color:#93a0b3;background:var(--p-paper)}
  .cn-foot{display:flex;align-items:center;gap:9px;margin-top:9px}
  /* right rail: the signal-capture widget, visibly a different product */
  .cn-rail{border-left:1px solid var(--ci-border);background:var(--p-paper);display:flex;flex-direction:column;
           overflow:hidden;font-family:var(--ci-inter)}
  .cn-rh{flex:none;display:flex;align-items:center;gap:8px;padding:10px 12px;background:var(--ci-tint);
         border-bottom:1px solid var(--ci-border)}
  .cn-rh .ic{width:20px;height:20px;flex:none;border-radius:5px;background:var(--ci);color:#fff;
             font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center}
  .cn-rh b{font-size:11.5px;font-weight:600}
  .cn-rh u{display:block;text-decoration:none;font-family:var(--pmono);font-size:7.5px;letter-spacing:.09em;
           text-transform:uppercase;color:#0b7c86}
  .cn-rb{flex:1;min-height:0;overflow:hidden;padding:11px 12px;display:flex;flex-direction:column;gap:10px}
  .cn-zone{border:1px dashed var(--ci-border);border-radius:8px;padding:9px 11px;background:#fff}
  .cn-zone .zl{font-family:var(--pmono);font-size:7.5px;font-weight:600;letter-spacing:.1em;
               text-transform:uppercase;color:#0b7c86;display:block;margin-bottom:6px}
  .cn-rrow{display:flex;align-items:center;gap:7px;font-size:10.5px;padding:4px 0;border-top:1px solid #eef1f5}
  .cn-rrow:first-of-type{border-top:0}
  .cn-rrow .nm{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .cn-rrow i{font-style:normal;font-size:9px;width:18px;height:18px;border-radius:4px;
             border:1px solid var(--ci-border);display:flex;align-items:center;justify-content:center;
             color:#93a2a3;background:#fff}
  .cn-rrow i.up{border-color:var(--ci);color:var(--ci);background:var(--ci-tint)}
  .cn-agg{font-size:10.5px;line-height:1.5;color:var(--ci-muted);background:var(--ci-tint);
          border-radius:7px;padding:8px 10px}
  .cn-agg b{color:var(--p-ink);font-weight:600}
  .cn-rf{flex:none;padding:8px 12px;border-top:1px solid var(--ci-border);background:#fff;
         font-family:var(--pmono);font-size:7.5px;letter-spacing:.07em;text-transform:uppercase;
         color:var(--p-mute)}
...
<div class="content">
  <div class="mast">
    ...
    distribution center. One warranty claim carries all three acts: nobody has written the process
    down yet, it gets mapped and built, it runs mostly on its own, and then it gets better.</p>
    ...
    <details class="flowbrief" id="flowbrief">
      ...
      <div class="fb">
        ...
        <ol>
          <li>Nothing has been surveyed. Maestro knows three processes because they run there, and
              warranty resolution is not one of them.</li>
          <li>Rather than start from nothing, start from a process UiPath has already mapped and had
              reviewed.</li>
          <li>One prompt plus the documents this company already had, and the analysis assembles
              itself. It asks the one thing no document could answer.</li>
          ...
          <li>The corrected design publishes as <b>one versioned, signed record</b> with an owner, a
              risk tier and a compliance scope, so the handoff can be audited.</li>
    ...
    <div class="capbox">
      ...
      <ol>
        ...
        <li><b>Automation Hub.</b> The design published as a governed, signed, versioned record.</li>
        ...
        <li><b>Cartographer, Suggestions.</b> The decision ledger, the suggestions it produces,
            applying one, and the estate ninety days on.</li>
      </ol>
    </div>
    ...
    <p class="pivot"><b>Open on Thursday:</b> which opening we use (this one, the finished estate, or
    the Use Case Explorer), whether the Automation Hub governance screen stays, and the runtime, which
    nobody has budgeted yet. The act runtimes below are my estimate, not an agreed number.
    <br><br><b>The cast, settled.</b> <b>Sarah Chen</b> decides coverage and is the one on stage.
    <b>Priya Raghunathan</b> owns the process and signs it off. <b>Tom Beckerman</b> administers
    claims. <b>Miguel Alvarez</b> takes the engineering exception. <b>Ryan Ochoa</b> owns quality and
    recurrence. <b>Kelsey Nordstrom</b> handles parts. There is no second Priya and no duplicated
    engineer role, and the Case Plan build thread uses the same six names.</p>

    <div class="viewsw">
      <span class="sw" role="group" aria-label="Storyboard layout">
        <button type="button" id="vFlow"  aria-pressed="true">Flow</button>
        <button type="button" id="vStrip" aria-pressed="false">Strip</button>
      </span>
      <span class="swnote">Strip compresses every scene to a thumbnail so a 21-scene flow fits on a screen or two. Click any screen to open it full size.</span>
    </div>
  </div>
...
function worklistView(){
  const rows = [
    ["WR-2026-0417","Coverage disputed — combined cause finding","Sarah Chen","No rule resolves a combined cause","Coverage and evidence review",true],
...
function suggestionsChrome(tab, crumb, body){
  const tabs = ["Feed","Improvements","Ledger","Dashboard","Settings"]
...
function ledgerView(){
  const rows = [
    ["hl","ok","Agreed","WR-2026-0417","PARTIAL + GOODWILL","Partial + goodwill","Sarah Chen","Coverage and evidence review","1:42 PM"],
...
function improvementView(){
  return suggestionsChrome("Improvements","Improvements <span class='sep'>/</span> IMP-0142",\`
    <div style="display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:13px;align-items:start">
      <div style="display:grid;gap:11px;min-width:0">
        ...
        <div class="ci-card pad">
          <span class="ci-lbl" style="margin-bottom:6px">Evaluation status</span>
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:6px">
            <span class="ci-chip ok">8 of 8 passed</span><span class="ci-count">7 from the golden set, 1 written for this change</span>
          </div>
          <p style="font-size:11px;color:var(--ci-muted)">Includes a replay of the decisions this was drawn from,
            and a case at $5,100 that must still reach a person.</p>
        </div>
      </div>
      <div style="display:grid;gap:11px">
        ...
        <div class="ci-card pad">
          ...
          <div class="ci-act"><span class="who">Eval agent</span>
            <span class="what">Validation passed, 8 of 8.</span></div>
        </div>
...
/* The injected signal-capture panel. Context and feedback only: the decision
   controls stay in the task form, so they are never shown twice. */
function augmentPanel(){
  return \`<aside class="aug" aria-label="Signal capture panel">
    <div class="ah"><span class="ic">C</span>
      <span><b>Why this is recommended</b><u>Cartographer · signal capture</u></span></div>
    <div class="ab">
      <p class="why">The part failed inside its warranty term, and separately somebody changed the machine's
        controls without approval. <b>Both contributed</b>, so full coverage and a flat denial are both wrong.
        Partial plus goodwill splits it and stays inside your authority.</p>
      <div>
        <span class="ci-lbl" style="margin-bottom:5px">Was this evidence useful?</span>
        <div class="erow"><span class="nm">Approved configuration baseline</span>
          <span class="tb"><i class="up">&#9650;</i><i>&#9660;</i></span></div>
        <div class="erow"><span class="nm">Controls change audit</span>
          <span class="tb"><i class="up">&#9650;</i><i>&#9660;</i></span></div>
        <div class="erow"><span class="nm">Third-party service report</span>
          <span class="tb"><i>&#9650;</i><i>&#9660;</i></span></div>
      </div>
      <p class="agg">Across similar claims, <b>78%</b> ended in partial plus goodwill. The 3 that were denied all
        had a disputed installation date.</p>
      <div class="opt2">Anything the buttons above don't capture? Optional, and it holds nothing up.</div>
    </div>
    <div class="af">Captured to the decision ledger · non-blocking</div>
  </aside>\`;
}
...
function mapEmptyView(){
  return cartographerChrome(\`<div class="mw">
    <div class="mw-top"><h4>Nothing surveyed yet</h4></div>
    <p style="font-size:12.5px;color:var(--p-mute);max-width:660px;line-height:1.6">Cartographer draws a map of the
      work this company actually does, process by process, owner by owner. Start from whatever already exists.
      Anything picked here is a starting point, not a commitment.</p>
    <div class="est" style="grid-template-columns:repeat(4,minmax(0,1fr))">
      <div class="d hi"><b>Modelled in Maestro</b><span class="m">3 found · 90 days of runs</span></div>
      <div class="d mid"><b>Industry seeds</b><span class="m">Manufacturing</span></div>
      <div class="d"><b>Documents &amp; recordings</b><span class="m">4 attached</span></div>
      <div class="d lo"><b>Nothing yet</b><span class="m">start from scratch</span></div>
    </div>
    <div class="pcard pad" style="max-width:680px">
      <span class="lbl" style="margin-bottom:6px">What Maestro cannot tell us</span>
      <p style="font-size:11.5px;line-height:1.55;color:var(--p-ink)">Maestro only knows the work that already runs
        there. Warranty resolution has no model, so it arrives as an empty dot. That is the point: it is the work
        nobody has mapped.</p>
    </div>
  </div>\`);
}
function mapWipView(){
  return cartographerChrome(\`<div class="mw">
    ...
    <div class="mw-legend">
      <span><i style="background:#0db4b9"></i>Case step, judgment and waiting</span>
      <span><i style="background:#3b6fe0"></i>Workflow step, runs start to finish</span>
    ...
    <div class="pcard pad">
      ...
      <div class="mw-item"><span class="n">01</span><span class="t"><b>Step 05 decides something and states no rule.</b>
        <u>Parts substitution. Who may approve an alternate, and against what standard? Routed to Kelsey Nordstrom.</u></span></div>
...
function mapDoneView(){
  return cartographerChrome(\`<div class="mw">
    ...
    <div class="mw-legend">
      <span><i style="background:#0db4b9"></i>Case step</span>
      <span><i style="background:#3b6fe0"></i>Workflow step</span>
...
function estateView(){
  const d = [["Warranty Ops","74%","hi"],["Support","81%","hi"],["Field Service","66%","hi"],
    ["Parts","52%","mid"],["Finance","58%","mid"],["Eng & Quality","31%","lo"],
    ["Commercial","24%","lo"],["IT","44%","mid"],["Aftermarket","12%","lo"]];
  return cartographerChrome(\`<div class="mw">
    <div class="mw-top"><h4>The estate, ninety days later</h4>
      <span class="mw-prog">134 processes · 47% mapped · 9 rules written down</span></div>
    <p style="font-size:12.5px;color:var(--p-mute);max-width:700px;line-height:1.6">The survey has run across the
      business. Warranty resolution, the one empty dot at the start, is now the best mapped process here, and it is
      the template the rest are measured against.</p>
    <div class="est">\${d.map(x=>\`<div class="d \${x[2]}"><b>\${x[0]}</b><span class="m">\${x[1]} mapped</span></div>\`).join("")}</div>
    <div class="pcard pad" style="max-width:760px">
      <span class="lbl" style="margin-bottom:6px">Ranked by attention owed</span>
      <div class="mw-item"><span class="n">01</span><span class="t"><b>Aftermarket, 12% mapped.</b>
        <u>Four processes decide something and record nothing. Not being watched at all.</u></span></div>
      <div class="mw-item"><span class="n">02</span><span class="t"><b>Commercial, 24% mapped.</b>
        <u>No named owner on three processes.</u></span></div>
      <div class="mw-item"><span class="n">03</span><span class="t"><b>Warranty Resolution, 74% mapped.</b>
        <u>Running, improving, and the only one with a decision ledger behind it.</u></span></div>
    </div>
  </div>\`);
}
...
function governanceView(){
  const nav = ["Workspace","Ideas","Process definitions","People","Components","Leaderboard",
               "Dashboards","Admin console"]
    .map(n=>\`<div class="ah-nav\${n==="Process definitions"?" on":""}">\${n}</div>\`).join("");
  const gov = [
    ["Process ID","AH-PRC-4712",true],
  ...
  const toc = [
    ...
    ["2.1 Trigger and entry channels",1,0],["2.2 In-scope / out-of-scope",1,0],
    ["2.3 Stakeholders",1,0],["3. Process Summary",0,0],["4. Process Detail",0,0],
    ...
    ["5. Conditional paths",0,0],["5.1 Waiting for customer evidence",1,0],
    ["5.2 Engineering exception",1,0],["5.3 Parts substitution review",1,0],
    ["5.4 Product-quality escalation",1,0],["6. Data and systems",0,0],
  ...
  return \`<div class="win" style="background:#fff">
    <div class="ah">
      <div class="ah-side">
        <div class="brand"><i></i>Automation Hub</div>
        \${nav}
      </div>
      <div class="ah-main">
        <div class="ah-top">
          <div class="ah-crumb"><b>Automation Hub</b><span class="sep">/</span>Process definitions
          ...
          <div class="ah-sub">v2 · Owner Priya Raghunathan · Updated Aug 25, 2026 ·
            published from Cartographer as one versioned entry</div>
        </div>
        ...
        <div class="ah-doc">
          ...
          <div class="ah-body">
            ...
            <p>Cobalt Ridge handles roughly <b>7,400 warranty claims a year</b> across industrial
              automation and material-handling equipment. Global Warranty Operations owns the
              process end to end, with Customer Support, Field Service, Parts and Logistics,
              Engineering, Quality and Finance joining at named stages. The trigger is a
              customer-reported failure through the hotline, email, portal or a technician; the
              close-out is a restored line, a validated customer outcome, and a reconciled cost
              posting.</p>
            <div class="ah-kpi">
            ...
            <p>Six stages always run. Four more open only on an exception, which is what makes
              ...
              ten agent tasks, seven processes, seven API calls and <b>twenty human decisions</b>.</p>
            <div class="ah-sign">
...
function consoleView(){
  const opts = [
    ["Deny coverage","Not supported: the bearing failed early on its own merits.",false],
    ["Approve full coverage","Not supported: it would absorb wear the customer caused.",false],
    ["Approve partial coverage plus goodwill",
     "Cobalt Ridge covers part and labour, the customer carries expedited freight. Splits along the two causes, inside her authority.",true]
  ].map(o=>\`<div class="cn-opt\${o[2]?" on":""}"><span class="rad"></span>
      <span style="min-width:0"><b>\${o[0]}</b><em>\${o[1]}</em></span></div>\`).join("");
  const facts = [
    ["Customer","Northstar Retail Distribution"],["Site","Joliet DC · sortation line 3"],
    ["Asset","SR-440 drive · SN 4471-882"],["In service","14 months · under warranty"],
    ["Claim value","$18,400"],["Line status","Down · 11 hr"]
  ].map(f=>\`<div><span>\${f[0]}</span><b>\${f[1]}</b></div>\`).join("");
  const cfg = [
    ["Belt speed","1.8 m/s","1.8 m/s","—",false],
    ["Drive torque limit","42 Nm","50 Nm","+19% · out of envelope",true],
    ["Ramp profile","Standard","Standard","—",false],
    ["Overload cutout","Enabled","Enabled","—",false]
  ].map(r=>\`<tr class="\${r[4]?"flag":""}"><td class="k">\${r[0]}</td><td>\${r[1]}</td><td>\${r[2]}</td>
      <td class="\${r[4]?"cn-delta":"k"}">\${r[3]}</td></tr>\`).join("");
  return \`<div class="cn">
    <div class="cn-top"><span class="lg">C</span>
      <span class="nm">Warranty Resolution Console</span>
      <span class="ptag gy">Coded app</span>
      <span class="who"><span><b>Sarah Chen</b><u>Warranty Resolution Lead</u></span><i>SC</i></span></div>
    <div class="cn-head">
      <div class="r1"><h4>WR-2026-0417</h4>
        <span class="ptag rd">Blocking</span><span class="ptag yl">Due today, 2:00 pm</span>
        <span class="ptag bl">Coverage and evidence review</span>
        <span class="pmono" style="margin-left:auto">SLA 4 HR · 3 HR 12 MIN ELAPSED</span></div>
      <div class="cn-facts">\${facts}</div>
    </div>
    <div class="cn-body">
      <div class="cn-col">
        <div class="cn-scroll">
          <div class="cn-why"><b>Why this reached you</b>
            <p>Two causes are established and they point to different payers. The drive failed early
            against its rated life, which is on us. Somebody also raised the torque limit past the
            approved envelope without sign-off, which is on the customer. Neither full coverage nor a
            flat denial fits, and allocating between them sits inside your delegated authority.</p></div>

          <div class="cn-ev"><div class="h"><span class="tri">▾</span><span class="ext">pdf</span>
            <span class="t">Third-party service report</span><span class="ptag rd">Premature failure</span></div>
            <div class="in"><p>Bearing assembly failed at <b>4,100 operating hours</b> against a
            <b>20,000 hour</b> rated life. Spalling on the inner race is consistent with fatigue, not
            contamination. The report calls this <b>premature failure of a component within its
            warranty term</b>. It also records the drive ran at <b>18% over rated torque</b> for six
            weeks before failure, which <b>materially shortened</b> the bearing's life without being
            the sole cause.</p></div></div>

          <div class="cn-ev"><div class="h"><span class="tri">▾</span><span class="ext">pdf</span>
            <span class="t">Approved configuration baseline</span>
            <span class="ptag yl">1 parameter out of envelope</span></div>
            <div class="in"><table class="cn-t">
              <tr><th>Parameter</th><th>As shipped</th><th>As found</th><th>Delta</th></tr>
              \${cfg}</table></div></div>

          <div class="cn-ev"><div class="h"><span class="tri">▾</span><span class="ext">log</span>
            <span class="t">Controls change audit</span><span class="ptag rd">No approval on record</span></div>
            <div class="in"><p class="cn-log">2026-05-02 06:14  <b>J. Alvarez</b> (Northstar, site controls)
torque_limit_nm  42 → 50
approval_ref  <span class="hi">none</span>
change_window  <span class="hi">outside scheduled maintenance</span></p>
            <p style="margin-top:7px">Cobalt Ridge holds no change request against this. Configuration
            outside the approved envelope <b>transfers consequential wear to the customer</b>, but does
            not void coverage on a component that was already failing early.</p></div></div>
        </div>
        <div class="cn-deck"><div class="cn-dec">
          <div class="dh"><b>Coverage decision</b>
            <span class="ptag tl">Agent recommendation preselected</span>
            <span class="pmono" style="margin-left:auto">CONFIDENCE MEDIUM-HIGH</span></div>
          <div class="db">\${opts}
            <div class="cn-rat"><span class="lbl" style="margin-bottom:5px">Rationale · recorded on the case</span>
              <div class="box">Both causes contributed. Bearing failed at 4,100 of 20,000 rated hours,
              and an unapproved torque increase of 19% accelerated it.</div></div>
            <div class="cn-foot"><span class="btn primary">Submit decision</span>
              <span class="btn sm">Send back for evidence</span>
              <span class="pmono" style="margin-left:auto">WRITES TO THE DECISION LEDGER</span></div>
          </div></div></div>
      </div>
      <aside class="cn-rail">
        <div class="cn-rh"><span class="ic">C</span>
          <span><b>Signal capture</b><u>Cartographer · injected widget</u></span></div>
        <div class="cn-rb">
          <div class="cn-zone"><span class="zl">Was this evidence useful?</span>
            <div class="cn-rrow"><span class="nm">Third-party service report</span>
              <i class="up">▲</i><i>▼</i></div>
            <div class="cn-rrow"><span class="nm">Approved configuration baseline</span>
              <i class="up">▲</i><i>▼</i></div>
            <div class="cn-rrow"><span class="nm">Controls change audit</span><i>▲</i><i>▼</i></div></div>
          <div class="cn-agg">Across similar combined-cause claims, <b>78%</b> ended in partial plus
            goodwill. The <b>3</b> that were denied all had a disputed installation date.</div>
          <div class="cn-zone"><span class="zl">Anything the buttons missed?</span>
            <p style="font-size:10.5px;color:#93a2a3">Optional, free text, holds nothing up.</p></div>
          <div class="cn-zone"><span class="zl">Captured automatically</span>
            <p style="font-size:10.5px;color:var(--ci-muted);line-height:1.5">Which evidence was opened
            and for how long, whether the recommendation was changed, and time to decide.</p></div>
        </div>
        <div class="cn-rf">Non-blocking · written to the decision ledger</div>
      </aside>
    </div>
  </div>\`;
}
...
// ---------- acts & scenes ----------
const ACTS = [
  {t:"Act I &middot; Build it", goal:"The cartographer agent maps how the work actually runs, the people who own it correct the map, the design becomes a governed record, and a coding agent turns it into a running case.", rt:"~7 MIN", scenes:[
    {actor:"Human", who:"Priya Raghunathan · Director, Global Warranty Operations", views:["mapempty"], status:"partial", short:"Nothing surveyed yet",
     ...
     title:"At the start, nobody has written this process down",
     narr:"THE OPENING IS STILL UNDECIDED. This is the empty-map version. Max wants to open on the finished estate instead and descend into warranty, and Raghu leans that way. Anvita's alternative is to open inside Delegate on the Use Case Explorer. All three are in this storyboard; pick one on Thursday.",
     tt:"This is the starting point. An empty map, and four ways to begin filling it. Maestro already knows about three processes, because those three actually run there, and it can hand us ninety days of run history for them. Warranty resolution is not one of them. It arrives as an empty dot, and that is the point: it is the work nobody has written down.",
     demo:["Open on the empty estate and name the four ways in.","Say warranty is the empty one, and that you will come back to this picture.","Decision needed: this frame, or open on the finished estate (scene 20), or on the Use Case Explorer (scene 02)."]},
    {actor:"System", views:["uce"], status:"build", short:"Or start from an attested process",
     frameLabel:"Cartographer · Use Case Explorer",
     title:"Or start from a process UiPath has already mapped",
    ...
    {actor:"Agent", views:["scribe"], status:"partial", short:"Sources read, analysis written",
     ...
     title:"One prompt in, and the analysis writes itself",
     narr:"A montage, not four separate demos. It reads the idea and every source, works out the stages and systems, asks the one thing the documents cannot answer, and publishes the analysis as a discovery artifact.",
     ...
     demo:["Run this in the real product wherever it exists.","Treat the whole beat as one montage. Do not stage four separate demos.","End on the stage list."]},
    ...
    {actor:"System", views:["mapdone"], status:"build", short:"Complete, and it says why",
     ...
     title:"Every step now names who owns it, what rule governs it, and whether anything gets recorded",
    ...
    {actor:"Human", views:["review"], status:"build", short:"They push back, the map changes",
     ...
     title:"The people who own the work correct it, and the map changes",
    ...
    {actor:"System", views:["governance"], status:"build", short:"The design becomes a governed record",
     frameLabel:"Automation Hub · Process definitions",
     title:"The design is published as a record with an owner, a risk tier and four signatures",
     narr:"NEW, and not something the group has reviewed yet. Max built this screen in his prototype and it is the piece that makes the handoff auditable: the PDD published as one versioned entry with an owner, a risk tier, a compliance scope, a review date and four sign-offs. Raise it on Thursday rather than assuming it is agreed.",
     tt:"Before any of this reaches a builder, it lands here. The design, the diagram and the solution architecture publish as one versioned entry in Automation Hub, and it stops being a document in a chat window. It has an owner. It has a risk tier and a compliance scope, because a warranty claim touches revenue recognition and customer data. It has a review date. And it has four names against it, from the people who corrected the map two screens ago. That is what makes the next step something an auditor can follow rather than something that just happened.",
    ...
    {actor:"Agent", views:["agentbuild"], status:"build", short:"The design goes to the coding agent",
     title:"The document stops being the deliverable and becomes the input",
    ...
    {actor:"System", views:["plan"], status:"build", short:"A case plan, not a folder of activities",
     ...
     title:"What the coding agent builds is a case, not a folder of activities",
  ...
  {t:"Act II &middot; Run it", goal:"Most claims finish without anybody. The few that reach a person arrive assembled, and what the person does there is captured. The act ends when the case does.", rt:"~4 MIN", scenes:[
    {actor:"System", views:["opsdash"], status:"build", short:"93 out of 100 need nobody",
     frameLabel:"Fleet-wide view &middot; Performance tab",
     title:"93 out of every 100 claims finish without a person",
    ...
    {actor:"Human", who:"Sarah Chen", views:["ac"], status:"built", short:"One decision, in a console",
     ...
     title:"Everything she needs to decide is on one screen, and what she marks useful is captured",
     narr:"Alin's ask, built: a console rather than a form. The three evidence sources are brought forward and readable rather than linked, so the combined-cause call is defensible. The decision lives in exactly one place, and the signal-capture rail never decides anything. A fuller standalone mock is in assets/case-app-console.html.",
     tt:"The agent has gathered the evidence, checked it against policy, and prepared a recommendation. The service report says the bearing failed at four thousand hours against a twenty thousand hour rating, which is on us. The controls audit says somebody raised the torque limit nineteen percent past the approved envelope with no sign-off, which is on them. Both are true, which is why neither full coverage nor a flat denial is right. She makes that call, and while she is here she marks which evidence actually helped. A few seconds, nothing held up, and it is the raw material for the last act.",
     demo:["Open the console. Point at the recommendation, then at the evidence that supports it.","Mark one piece of evidence useful. Keep it to about three actions.","Submit the decision.","Don't show decision controls in both the rail and the form. The rail never decides."],},
    ...
    {actor:"System", views:["audittrail"], status:"build", short:"The execution trail, and the case finishes",
     ...
     title:"The trail shows what triggered the reroute and who signed off, then the case closes",
  ...
  {t:"Act III &middot; Improve it", goal:"Every decision people made along the way is on the record. Reading across it produces suggestions, one gets applied as a rule, and the map itself gets better.", rt:"~4 MIN", scenes:[
    ...
    {actor:"Human", views:["improvement"], status:"build", short:"Approve the rule once",
     ...
     narr:"A business rule added to the case workflow, which is what makes a single Apply honest. It takes effect at run time with no redeployment, it can be withdrawn, and one in ten is audited. This is where 93 becomes 96.",
     tt:"So I take the suggestion. The platform drafts the change, runs it against the existing checks plus one written for this change, and shows me what it would do. It is a business rule on the case workflow, which means it takes effect at run time with no redeployment and I can take it back if I am wrong. Eight of eight passed, including a claim just over the threshold that still has to reach a person. And here is the number I asked you to hold on to. Ninety three percent becomes ninety six. I approve this rule once, instead of approving every claim it covers.",
    ...
    {actor:"System", views:["estate"], status:"build", short:"The estate, ninety days on",
     ...
     narr:"Key frame, and the callback to scene 01. The empty dot is now the best mapped process here, and the rule Sarah's decisions produced is written down in the map. If the opening changes to estate-first, this is the frame that moves to the front.",
     ...
     demo:["Put this next to scene 01 if you can. The callback is the payoff.","Say the loop closes back into the map.","If the opening becomes estate-first, this frame leads instead."]},
...
const VNAME = {scribe:"Cartographer · PDD", agentbuild:"Coding agent · build proposal", plan:"Case plan (design canvas)", liveedit:"Live edit · change summary",
  ...
  eventreassess:"Case App · case detail", audittrail:"Instance management", close:"Closing", governance:"Automation Hub · Process definitions", mapempty:"Cartographer · empty estate", mapwip:"Cartographer · map of work, in progress", mapdone:"Cartographer · map of work, complete", review:"Cartographer · review", uce:"Cartographer · Use Case Explorer", estate:"Cartographer · the estate", ledger:"Cartographer \\u00b7 Suggestions \\u00b7 Ledger", suggestions:"Cartographer \\u00b7 Suggestions \\u00b7 Feed", improvement:"Cartographer \\u00b7 Suggestions \\u00b7 Improvement"};
const VMETA = {
...
const NATIVE_VIEWS = new Set(["agentbuild","liveedit","scribe","plan","casemanager","ledger","suggestions","improvement","mapempty","mapwip","mapdone","review","uce","estate","governance"]);
...
ACTS.forEach((act,ai)=>{
  ...
  tocHtml += \`<div class="tgroup" data-act="\${ai}">
    <a href="#act\${ai}"><span class="tn">ACT\\n\${"I II III".split(" ")[ai]}</span><span class="tt">\${act.t.replace(/^Act [IV]+ · /,"")}</span><span class="tm">\${act.rt.replace("~","").toLowerCase()}</span></a>
...
(function(){
  const bFlow = document.getElementById("vFlow"),
        bStrip = document.getElementById("vStrip"),
        KEY = "fusion.mergedView";
  function apply(mode, remember){
    const strip = mode === "strip";
    document.body.classList.toggle("view-strip", strip);
    bFlow.setAttribute("aria-pressed", String(!strip));
    bStrip.setAttribute("aria-pressed", String(strip));
  ...
  apply(saved === "strip" ? "strip" : "flow", false);

  /* remember whether the plain-language walkthrough is open */
  const fb = document.getElementById("flowbrief"), FK = "fusion.mergedBrief";
  try{ if(localStorage.getItem(FK) === "1") fb.open = true; }catch(e){}
  fb.addEventListener("toggle", ()=>{ try{ localStorage.setItem(FK, fb.open?"1":"0"); }catch(e){} });
})();
...
<div class="content">
  <div class="mast">
    ...
    <div class="viewsw">
      ...
      <span class="swnote">Strip puts each act on one line. Click any screen to open it full size.</span>
    </div>
  </div>
...
const ACTS = [
  ...
  {t:"Act II · Run it", goal:"Most cases finish without anybody. The few that reach a person arrive assembled, with a recommendation and the reasoning behind it, and what the person does there is captured.", rt:"~2 MIN", scenes:[
    ...
    {actor:"Human", who:"Sarah Chen", views:["ac"], status:"built", short:"The recommendation, and why",
     frameLabel:"Runtime · Case App · task, with the signal-capture panel",
     title:"The recommendation is already there, and so is the reasoning behind it",
     narr:"The evidence is assembled, the policy is evaluated, and the recommendation is there with its rationale before she opens anything. The panel on the left is the part that matters later: it shows why this was recommended, and it lets her mark which evidence helped. Three actions, none of them blocking, and the decision controls stay in the form so nothing is offered twice.",
     tt:"The agent has already gathered the evidence, checked it against policy, and prepared a recommendation. The panel here tells her why. The part failed inside its warranty term, and separately somebody changed the machine's controls without approval, so both sides contributed and neither full coverage nor a flat denial is right. She spends her time on that call instead of rebuilding the case out of five systems. And while she is here she marks which evidence actually helped her. That takes a few seconds, it holds nothing up, and it is the thing that makes the next part possible.",
     demo:["Open the task. Point at the recommendation, then at the panel explaining why.","Mark one piece of evidence useful. Keep it to about three actions.","Submit the decision.","Don't show decision buttons in both the panel and the form. The panel never decides."],}
  ]},
...
(function(){
  ...
  /* remember whether the plain-language walkthrough is open */
  const fb = document.getElementById("flowbrief"), FK = "fusion.sbBrief";
  try{ if(localStorage.getItem(FK) === "1") fb.open = true; }catch(e){}
  fb.addEventListener("toggle", ()=>{ try{ localStorage.setItem(FK, fb.open?"1":"0"); }catch(e){} });
})();
